...
const resolvers = {
  ...
  Mutation: {
    ...
    deleteUser: (parent, args, context, info) => {
      ...
      pubSub.publish("userDeleted", { userDeleted: deletedUsers[0] });
    ...
    deleteEvent: (parent, args, context, info) => {
      ...
      pubSub.publish("eventDeleted", { eventDeleted: deletedEvents[0] });
    ...
    updateParticipant: (parent, args, context, info) => {
      let participantIndex = participants.findIndex(
        (participant) => participant.id === args.input.id
      );
      ...
      pubSub.publish("participantUpdated", {
        participantUpdated: participants[participantIndex],
      });
    ...
    deleteParticipant: (parent, args, context, info) => {
      ...
      pubSub.publish("participantDeleted", {
        participantDeleted: deletedParticipants[0],
      });
    ...
    updateLocation: (parent, args, context, info) => {
      let locationIndex = locations.findIndex(
        (location) => location.id === args.input.id
      );
      if (locationIndex === -1) {
      ...
      pubSub.publish("locationUpdated", {
        locationUpdated: locations[locationIndex],
      });
    ...
    deleteLocation: (parent, args, context, info) => {
      ...
      pubSub.publish("locationDeleted", {
        locationDeleted: deletedLocations[0],
      });
